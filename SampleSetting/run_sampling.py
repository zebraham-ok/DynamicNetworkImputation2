...
def run_single_training(
    ...
    # full_dataset). The validation split drives checkpoint selection, the composite score
    # (0.5 * FactSet quantile + 0.5 * validation AUC) and early stopping; the test split is only
    # reported (see Training/trainer_common.py, train()).
    ...
    # Train (checkpoint selection: 0.5 x FactSet quantile + 0.5 x validation AUC)
...
def _load_result_from_checkpoint(checkpoint_path: str) -> dict:
    ...
    # Composite score = 0.5 * factset_quantile + 0.5 * AUC of the selection split. Runs produced
    # before the validation-based selection switch carry no val_auc, so they fall back to test_auc.
