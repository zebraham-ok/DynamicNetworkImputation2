...
Note: the Bootstrap data loader builds a 1:1 training set (dynamic negatives plus the CSV
predefined negatives assigned to train) and a static-only val/test at 1:1, which additionally carry
their 0.1 share of the CSV predefined negatives when `data.use_pred_neg` is enabled.
...
def load_model_config(config_name):
    """Load the model architecture config: Training/common_config.yaml <- Models/configs/{name}.yaml

    The parts shared by every backbone (e.g. the node-feature extractor) live in
    Training/common_config.yaml, so the same merge as Training/config_loader.load_config is applied;
    reading the model YAML alone would silently fall back to the code defaults for those keys.
    """
    ...
    if os.path.exists(yaml_path):
        ...
        common_cfg = _load_yaml(os.path.join(ROOT_DIR, 'Training', 'common_config.yaml'))
        return deep_merge(common_cfg, _load_yaml(config_name))
...
def main():
    ...
    bootstrap_data = create_bootstrap_datasets(
        ...
        neg_dir=data_cfg.get('neg_dir', None),
        toy_mode=toy_mode,
    ...
    for b in range(num_bootstrap):
        ...
        if scratch_mode and b == 0:
            ...
            backbone_prefixes = ('static_encoder.',)
            if hasattr(model, 'static_gnn') and not hasattr(model, 'static_encoder'):
                backbone_prefixes = ('static_gnn.',)
            # The shared node-feature extractor belongs to the frozen backbone
            backbone_prefixes = backbone_prefixes + ('feature_extractor.',)
