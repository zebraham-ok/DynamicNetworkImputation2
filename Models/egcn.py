...
class EGCN_PyG_Vectorized(nn.Module):
    """Vectorized version of EGCN, precomputes all adjacency matrices, optional FC feature reduction"""
    def __init__(self, dynamic_data, hidden_dims, activation=nn.ReLU(), 
                 num_gru_layers=2, dropout=0.3, time_steps=None, device=None,
        ...
        # Optional FC dimension-reduction layer
        self.use_fc_embedding = use_fc_embedding
        if use_fc_embedding:
            self.fc_embed = FCEmbedding(node_feat_dim, fc_embed_dim, dropout).to(self.device)
    ...
    def forward(self, node_pairs, time_indices):
        ...
        if self.fc_embed is not None:
            node_feats = self.fc_embed(self.raw_node_feats)
        else:
            node_feats = self.raw_node_feats
