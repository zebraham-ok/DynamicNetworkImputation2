...
- **两因素的可比性不同，别混着下结论**：
  ...
  - A（度）本来就是**传导式特征**（2026-09-17 之前度由全窗口正样本算得，含 val/test 边），而且它就是 `min_degree=2` 过滤时用的那个量 ⇒ 该列直接编码"样本是否满足入样规则"。实测（`_tmp_dump/leak_degree.py`，全窗口计数下）：只把 `min(log1p(deg_u), log1p(deg_v))` 当分数，对 val/test 固定负样本就有 **AUC 0.984**（扣掉样本自身那条边仍 0.936），而带度模型的 test AUC 只有 0.975(ftf)/0.9885(fff) ⇒ **"增益"全部由这一列解释**。因此开度的 run 不能与关度的 run 在 val/test 指标上并列，也不能进结果表；且 X 宽了一列 ⇒ 旧 ckpt / `backbone.pth` 全部失效，Phase 1 必须重跑。
- **2026-09-17 起 A 格必须在 `degree_scope: train_only` 下重跑**：这一列现改为运行期派生，**不再写库**，默认只数那 80% 训练池（与静态图只用 train_pool 边传播同口径），所以它不再是"入样规则"的等价物 ⇒ 上面那组泄漏数字描述的是**旧口径**，新口径下探针结论要重新表述。`train_only` 仍含"自计数"，且部署一个这样训出来的 ckpt 时要走 `calibrated_full`（§3.5、§8 陷阱 19/19b）。
- 度**每次运行现算**（2026-09-17 起），不再写回 Neo4j 属性 `degree_property`：旧的"首次写库、之后只读"行为已被移除（它让度变成跨 run、跨入口共享且无法追溯来源的全局状态）。多出来的开销只在 `train_only` 下算训练池那一次（数万条三元组，毫秒级）；部署侧用 `full` 要扫一遍窗口内全部 semi 正样本（约 200k 行，秒级），`source_filter` 为空时会扫全库（插补库 10^8 关系，有 WARNING）。
...
create_dataloaders(
    ...
    # 2026-09-17 起（度通道改为运行期派生）：下面 6 个键由 dataset_feature_kwargs() 透传，
    # create_bootstrap_datasets / CompanySupplyDataset 的签名必须同步，否则会在入口 TypeError
    # （已发生过：2026-09-17 run_sampling.py "unexpected keyword argument 'degree_scope'"）
    degree_scope='train_only', degree_triples=None, degree_triples_path=None,
    degree_thinning=1.0, degree_thinning_seed=None,
...
特征开关统一由 `dataset_feature_kwargs(ds_cfg)` 收集，保证训练与推理的 X 布局不会漂移。**给这个函数加键时，必须同时给所有接收它的入口加同名形参**——`CompanySupplyDataset.__init__` / `create_dataloaders` / `create_bootstrap_datasets` 三个签名由自检强校验：`_tmp_dump/degree_scope_selftest.py` 第 8 节（`inspect.signature` 求子集），漏改会在自检而非训练启动 1 小时后报错。
...
### 3.5 节点特征：度通道 `dataset.use_attr_degree`（2026-09-15 新增；2026-09-16 二次修订后默认**关**，只作 opt-in 探针；**2026-09-17 重写为"运行期派生 + 显式 scope"**）
...
- 开关：`Training/common_config.yaml → dataset.use_attr_degree`（**默认 `false`**：d=701 是基线；`true` 只在 `Models/configs/{gatgru_vec,gatgru_2layer}_deg[_down].yaml` 里显式打开 ⇒ X 宽 702，d=701 的旧 ckpt 不可加载）。`Bootstraps/bootstrap_v2_config.yaml` **不继承** common，需手工同步（已同步为 `false` + 下表的 scope 四项，两侧必须同值）。
- 度的定义：**两端都计入、按 (source, target, year) 三元组计次数**（含跨年重复），受 `dataset.source_filter` 约束 —— 与 `min_degree` 过滤用的是同一个量。**不等同于"不同交易对手个数"**。某一端没有 embedding 的三元组整条跳过（与旧查询 `WHERE c1.emb IS NOT NULL AND c2.emb IS NOT NULL` 一致）。
- **2026-09-17 起不再是 Neo4j 属性**：这一列是**每次运行现算的派生特征**，既不读也不写 `degree_property`（该键保留仅为让旧 yaml 仍能解析）。旧行为是算完以整数写回节点属性 ⇒ 无法复现、无法版本化、跨 run 与跨入口共享的全局可变状态。清理历史遗留：`MATCH (c:EntityObj) WHERE c.degree IS NOT NULL REMOVE c.degree`（若从没启用过就没有该属性）。
- 计算时机：`full_dataset` 构造时**不**计算（`train_only` 需要划分结果），改为 `assemble_node_features()` 惰性调用 `ensure_degree_values()`；`create_bootstrap_datasets()` 在拿到 `train_pool_pos` 后**立即**算并挂到 `full_dataset`，再由 `_share_metadata` 分发给 val/test —— **三个 split 读到的是逐位相同的一列**，与静态图只用 train_pool 边做传播同口径。

#### `dataset.degree_scope`：这一列能看到哪些边

| scope | 计数来源 | 何时合法 |
|---|---|---|
| `train_only`（**训练侧默认**） | 只数交给 `set_degree_triples()` 的三元组（学习管线里就是那 80% 训练池） | **评测期唯一可引用的设定**。`*_deg` 消融必须用它 |
| `full` | 窗口内全部正样本（等价于 2026-09-17 之前的行为，只是不再写库） | **部署期**（插补）。禁止用于给 val/test 打分 |
| `calibrated_full` | 同 `full`，再按 `degree_calibration_path` 里训练期那一列的经验分布做分位数映射 | 部署一个用 `train_only` 训出来的 ckpt 时的**推荐**设定 |

配套键：`degree_triples_path`（划分外的独立入口用 CSV 指定训练三元组）、`degree_thinning` ∈ (0,1]（按 keep 概率抽稀后计数，用来让模型见过多种"图的完整程度"，默认 1.0）、`degree_thinning_seed`、`degree_calibration_path`、`degree_calibration_out`（训练时顺手导出参考分布给以后部署用）。

#### 为什么部署期必须用全图（`full` / `calibrated_full`）

- 部署时**没有被留出的事实边**：候选对本身就是未观测的对，它的那条待预测边不在观测图里，所以不可能给自己的特征 +1（不存在自泄漏；`train_only` 若在评测期使用则有，因为 val/test 边实实在在被数进去了）。
- 部署的消息传播邻接本来就是全图：`Prediction/get_imputation_{fast,tensor}.py` 里 `build_static_graph(full_set=full_dataset, edge_info_dataset=full_dataset)`，且推理侧 `data.min_degree` 默认 0（不过滤边）。度只是这个邻接的确定性函数 ⇒ 两者必须同口径，否则模型会看到训练期从未出现过的 (邻接, 特征) 组合。
- 上一轮写回的插补边是 `source='imputation'`，被 `source_filter='semi'` 排除 ⇒ 跨轮插补不会自我膨胀。
- 代价是 **train→deploy 漂移**：训练那一列是被抽稀过的（≈ 观测度的 Binomial(d, 0.8)，相对 sd ≈ 0.5/√d，且 `log1p` 只在低端放大）。要么用 `degree_thinning < 1` 多训几个完整度，要么部署时走 `calibrated_full`（`map_to_reference()` 单调、保序、处理并列值，把全图度映射回训练期尺度）。
- 守卫：`ensure_degree_ready_for_deployment()` 在 `train_only` 下**直接报错**（部署没有"训练池"这个概念，重建不出来），在 `full` 且没给参考分布时打 WARNING。`Prediction/imputation_common.yaml` 已显式覆盖 `degree_scope: full`（common 默认是 `train_only`，必须覆盖，不能靠继承）。

#### 陷阱（写论文前必读）

- 这一列**就是 `min_degree=2` 过滤用的那个量**，所以**全窗口**计数会直接编码"样本能否入样"。**实测泄漏量级**（2026-09-16，`_tmp_dump/leak_degree.py`，本地 7687 复算，正样本 80,965 条 vs 项目口径 80,839）：正样本 `min(deg_u,deg_v)` 恒 ≥2（p50=11），而 val/test 固定负样本（= 有 embedding 公司间的**均匀随机对**）有 **88.7%** 的 `min(deg_u,deg_v) < 2`；于是
...
- **`train_only` 消掉了上面最强的那条泄漏链**（列不再等于入样规则），但**没有**消掉"自计数"：训练时被预测的那条边仍在 `train_pool` 里给两端各 +1。要彻底消掉只能做 leave-one-out（按样本扣自身那条边），那会让度从"节点静态列"变成"逐样本特征"，得重写 X 的接口，当前未实施。写论文时把这一句讲清楚即可。
- **更底层的一条**：即使完全没有度通道，`min_degree=2` 仍然是按**全正样本**决定"哪些节点进得来"。这是**入样规则层面**的问题，改它会牵动所有历史 run 的可比性，目前只作为 limitation 说明。
- **`*_deg` 的旧数字必须重跑**：它们来自泄漏的全窗口计数；`train_only` 下该列的含义已变（不再等价于入样规则），探针结论要重新表述。
...
- 自检：`D:\Coding\envs\Torch\python.exe -X utf8 _tmp_dump\degree_scope_selftest.py`（计数口径、抽稀确定性、参考分布保存/分位数映射、部署守卫，24 项，无需连 Neo4j）。
...
| `dataset.use_attr_degree`, `degree_scope` | common（**默认 `false` / `train_only`**；只有 `_deg*` 消融 yaml 显式改 `true`；`Bootstraps/bootstrap_v2_config.yaml` 不继承 common、需手工同步同值；`Prediction/imputation_common.yaml` 必须显式覆盖成 `full`） | `dataset_feature_kwargs` → `ensure_degree_values` | X **+1 列** `log1p(degree)` ⇒ 旧 ckpt 不兼容；列随 `degree_scope`（train 池 / 全图 / 全图+标定）而变，且不再读写 Neo4j 属性（2026-09-17）⇒ 属**标签可分离性探针**，**开关两侧不可在 val/test 上并列比较、不进结果表**，`_deg` 的旧数字要重跑（§3.5、§1.3） |
...
19. **度通道是 opt-in 探针，不是默认基线（2026-09-16 二次修订；2026-09-17 重写为运行期派生）**：`dataset.use_attr_degree` 默认 `false` ⇒ X 宽 701，只有 `_deg*` 消融 yaml 显式打开（X 宽 702，d=701 的旧 ckpt 不可加载）。理由是实测该列**单独**就足以把 val/test 分开（单列 AUC 0.9845、扣掉自身边仍 0.9364，见 §3.5），而它带来的对象是**全窗口**计数（度由含 val/test 边的全部正样本算得）。因此 ① 开度的 run 不能与关度 run 在 val/test 上并列，更不能进结果表；② 该开关的开与关同样不可并列（它只能回答"度信息有没有用"，不是"更强的模型"）。2026-09-17 起这一列**不再写库**，并引入 `dataset.degree_scope`：**评测期只能用 `train_only`**（只数训练池），`full`（全图）只允许**部署期**，`train_only` 在插补入口会**直接报错**、`Prediction/imputation_common.yaml` 必须显式覆盖成 `full`（common 默认 `train_only`，不能靠继承）。⇒ `*_deg` 的旧数字全部来自泄漏的全窗口计数，**必须重跑**。同理 `source_filter` 为空时会扫全库（插补库 10^8 关系），见 §3.5。
19b. **即使 `train_only`，度里仍含"自计数"**：训练时被预测的那条边自己在训练池里给两端各 +1 ⇒ 这一列严格说每个正样本都"看见了自己"。彻底消除需要按样本做 leave-one-out（会把度从节点静态列变成逐样本特征，需重写 X 接口，当前未实施）；写论文时把它和"更底层的 `min_degree` 入样规则仍然按全正样本决定"一起作为 limitation 说明（§3.5）。
...
| 2026-09-18 | **【数据正确性】修 `run()` 打分循环"片内下标漏加片偏移"**：`hit_pos = flatnonzero(probs > thr)` 是**片内**下标（0..`batch_size`），而 `i/j/k` 是**整个 chunk** 的张量 ⇒ 每个 slice 都用 `i[hit_pos]` 取回 chunk 的**前 `batch_size` 行**。后果：① 同一 `(src,tgt,year)` 被反复 enqueue ⇒ MERGE 全打到已有边上，实测 `[Writer-account] sent=792,502 / matched=792,502 / created=513`（`created` 恒为 513 ≈ 一个 batch 的命中数，不随 sent 增长）；② **写回的 `probability` 与端点不属于同一个候选对**（`hit_prob = probs_np[hit_pos]` 是片内、正确，端点却取自别的行）。上一轮全量跑落库的 85,839 条 `egcn-deg` 边 ≈ 514 × 167 chunk ⇒ 只是每个 1M chunk 的前 ~512 个候选且分数错位，`162M → 85,839` 由此而来（`dropped=0` 已排除 id 不命中；MERGE 键含 `source:'imputation'` 与 factset/semi 不合并） | `Prediction/get_imputation_tensor.py`、`_tmp_dump/imput_slice_offset_selftest.py`（新增自检） | 只加 `+ s`：`hit_t = torch.as_tensor(hit_pos, ...) + s`（`hit_prob = probs_np[hit_pos]` 仍为片内、不变）；修复后每个候选对恰好发出一次（自检 distinct 455/455，旧版 77/455、dup 5.9×）。**接口无变化**；对已落库数据是破坏性的：重跑前须 `delete_imputation_edges.py --model egcn-deg`（先 `--dry-run` 计数）。`get_imputation_fast.py`（list-of-dict 版）按 `batch[j]` 直接取字典，**无此 bug**；两个脚本的候选顺序本就逐位相同 | §2.3, §9 |
| 2026-09-18 | **新增「逐模型 × 逐采样模式选最佳 ckpt + 阈值」收集脚本**：扫描 `<root>/<model>/sampling_*/seed*/<i>_<flag>_<role>/`（同一套代码兼容 `ensemble/<model>/` 布局），每个 (模型, flag) 按 `--criterion`（默认 `test_auc`，即 `Prediction/imputation_common.yaml → prediction.threshold_by_model` 的既有选模口径）排出 4 seed × {scratch,frozen} 并给出最佳 ckpt；阈值优先读该 run 自带的 `model_predictions_best_auc_thresholds.json`，缺失时从 `model_predictions_best_auc.npy` 重算（youden = max(tpr−fpr)、F1 网格与 `Training/utils.F1_SCAN` 同参数） | `Analysis/collect_best_ckpt_thresholds.py`（新增）；产物 `results/<数据集>/best_ckpt_thresholds.json` | 输出含 `best` / `ranking` / `other_ckpt`（`best_model.pth` 对照，提醒阈值只对 `best_auc` 有效，因为 `seed_ci_figures.py` 读的就是 `model_predictions_best_auc.npy`）/ `warnings`（未解包的 `.zip`、缺 summary.yaml、缺阈值文件）；CLI：`--root --out --criterion {test_auc,val_auc,fsq_test,youden_j} --ckpt-tag {best_auc,best} --flags` | §9 |
| 2026-09-18 | **插补写回的三层计数 + 开跑前预检（诊断 "写入 3.07e7 / 入库 8.6 万" 缺口）**：`written` 此前在 `execute_query` 后按 `len(batch)` 累加、丢弃 Cypher 的 `RETURN count(r)`，只代表"发出的行数"，掩盖两种丢失：① `MATCH (u),(v) WHERE id(u)=row.src` 未命中 ⇒ 该行被 Cypher **静默丢弃**（不报错）；② MERGE 命中同 `(src,tgt,year,source,model)` 的边 ⇒ 只更新不新增（MERGE 键含 `source:'imputation'` ⇒ 与 factset/semi 边**不会**合并）。现在统计 **sent / matched / created**，语句改 `ON CREATE SET r.__impute_new = true` + `WITH ... REMOVE r.__impute_new` + `RETURN count(r) AS matched, sum(CASE WHEN is_new ...) AS created`；开跑前用第一个可用块同步写 1–2 条做预检（id 是否在本库命中 + 是否真的变成新边），失败即中止，不再空跑数小时；`start()`/`stop()` 各数一次库内该 model 的 imputation 边数并给出 delta 与"计数 vs 实际"是否一致的判定；命中率 > 50% 打印阈值告警（实测两帧 30.9M/30.9M、35.9M/36.1M ⇒ 阈值几乎没在筛） | `Prediction/get_imputation_tensor.py` | 新增 `Neo4jBatchWriter.count_edges()` / `preflight()` / `_execute_and_account()` 与 `_LEGACY_WRITE_QUERY_TEMPLATE`（诊断语句被服务端拒绝时自动回退旧语句，代价是 created 与 merged 不可分）；`_write_single` 改为复用同一语句（1 行 UNWIND）以保证计数口径一致；CLI 新增 `--no-preflight`；配置 `prediction.writer_preflight`（默认 true，`--dry-run` 下自动关）；**输出边 schema 不变**（`__impute_new` 在同语句内 REMOVE）；进度条 `written` 语义不变（仍为 sent），新增 `matched`/`created` 只在 `stop()` 与每 30 s 的 `[Writer-account]` 行打印 | §2.3, §9 |
| 2026-09-17 | **修复嵌入缓存 `compute_embeddings` 的维度错配（GAT-GRU\* 缓存一直静默失效）**：`compute_embeddings` 原来把 `model.raw_node_feats`（701）直接喂 `static_encoder`，而带 `feature_extractor` 的 backbone 里 GAT 首层输入是 `fc_embed_dim`（128）⇒ `mat1 and mat2 shapes cannot be multiplied (51509x701 and 128x512)`（服务器上 gatgru_2layer 实测）⇒ 异常被 `precompute_embeddings` 的 `except` 吞掉后**静默退回逐 batch 全图 forward**（慢 1–2 个数量级）。新增模块级 `apply_backbone_feature_extractor(model,x)`（取 `model.feature_extractor`，回退 `model.fc_embedding`，都没有则原样返回），`compute_embeddings` 的三条分支统一改用它；`precompute_embeddings` 的 `except` 分支新增 `[WARN]`（缓存失败不再是静默降级，会点明"每个 batch 重跑全图编码器"并建议 Ctrl-C 或加大 `batch_size`） | `Prediction/get_imputation_tensor.py` | 纯修复：只影响嵌入缓存路径的**输入张量**，与 backbone 自身 `forward()` 走同一条 `apply_node_feature_extractor` 语义；无 extractor 的 backbone（GAT conv0 权重 512×701）行为逐字不变。自检 `_tmp_dump/verify_emb_cache_offline.py`（合成图，零 Neo4j）：复现原报错 → 修复后 `compute_embeddings` 产出 `[13,N,128]`；缓存路径 vs `model(node_pairs,years)` **逐位一致**（max Δ = 0（bs=32）/ 1.19e-7（bs=2048））；无 extractor 分支确认为 no-op | §2.3, §8, §9 |
...
| 2026-09-17 | **度通道重写为"运行期派生 + 显式 scope"（Neo4j 里不再有 `degree` 属性）**：原先度由全窗口所有正样本算得并**永久写回**节点属性 `degree_property`（存在性检查只看"有多少节点带该属性"，没有任何指纹 ⇒ 换 `source_filter` / 换窗口 / 插补写回库之后都会静默复用旧值，且跨 train / bootstrap / 插补三个入口共享可变全局状态）。新口径：① 每次运行现算，不读不写 Neo4j（`degree_property` 降为保留键）；② `dataset.degree_scope` 三档 —— `train_only`（默认，**只数训练池**，与静态图只用 train_pool 边做传播同口径；train/val/test 经 `_share_metadata` 共用同一列）、`full`（全图，= 旧行为；**只允许部署期**）、`calibrated_full`（全图 + 按训练期参考分布做保序分位数映射）；③ `full_dataset` 构造期不再算度（那时还没有划分），改由 `create_bootstrap_datasets` 拿到 `train_pool_pos` 后立刻算并挂到 `full_dataset`，`assemble_node_features` 侧保留惰性兜底；④ 新增配套键 `degree_triples_path` / `degree_thinning` / `degree_thinning_seed` / `degree_calibration_path` / `degree_calibration_out`（见 §3.5 的键表）；⑤ **部署期必须用全图**：候选对未观测 ⇒ 不存在自泄漏，且推理时消息传播的邻接本来就是 `build_static_graph(full_set=full_dataset, edge_info_dataset=full_dataset)`、`data.min_degree` 默认 0，度是该邻接的确定性函数 ⇒ 两者不同口径会让模型看到训练期没见过的 (邻接, 特征) 组合；写回的插补边是 `source='imputation'`、被 `source_filter='semi'` 排除 ⇒ 不会跨轮自我膨胀；⑥ `Prediction/imputation_common.yaml` 显式覆盖 `degree_scope: full`（common 默认 `train_only`，不能靠继承），两个插补入口调用新增的 `ensure_degree_ready_for_deployment()`：`train_only` 直接报错、`full` 无参考分布时打 WARNING；⑦ `Prediction/find_threshold*.py` 通过新增的 `rebuild_degree_column()` **重算**（而非持久化）训练期那一列（确定于 all_pos + ratios + seed），从而与它要复现的那次评估逐位一致 | `Data/graph_dataset.py`、`Data/company_dataset.py`、`Training/common_config.yaml`、`Bootstraps/bootstrap_v2_config.yaml`、`Prediction/imputation_common.yaml`、`Prediction/get_imputation_{fast,tensor}.py`、`Prediction/find_threshold{,_per_model}.py`、`TechnicalGuide.md` | `CompanySupplyDataset` 构造新增 6 个可选参数（`degree_scope` 非法值 `ValueError`、`degree_thinning ∉ (0,1]` `ValueError`）；新增模块级常量 `DEGREE_SCOPE_*` 与纯函数 `count_degree_from_triples` / `thin_triples` / `load_degree_triples_csv` / `save_degree_reference` / `load_degree_reference` / `map_to_reference` / `ensure_degree_ready_for_deployment`；**删除** `_ensure_degree_property()`（写库路径整体消失），新增 `set_degree_triples` / `compute_degree_values` / `ensure_degree_values` / `_resolve_degree_triples` / `_full_window_triples`；`dataset_feature_kwargs` 新增 6 键；`create_bootstrap_datasets` 在划分后注入三元组；`_share_metadata` 同步新字段；`company_dataset` 新增 `rebuild_degree_column()`；`describe_node_features()` 输出带 scope。**计数口径与旧查询逐位一致**（两端累计三元组的次数；任一端无 embedding 则整条跳过，对应旧 `WHERE c1.emb IS NOT NULL AND c2.emb IS NOT NULL`）。**兼容性**：`use_attr_degree: false`（默认）行为完全不变；`_deg*` 系列数字全部要重跑（`train_only` 下该列不再等于 `min_degree` 入样规则，最强那条泄漏链被切掉，**自计数**仍在 —— 彻底消除需按样本做 leave-one-out，那会把度从节点静态列变成逐样本特征并重写 X 接口，未实施）。自检 `_tmp_dump/degree_scope_selftest.py`（24 项，零 Neo4j） | §3.5, §7, §8 陷阱 19, §9 |
| 2026-09-17 | **新增 `Models/configs/egcn_deg.yaml`**：把度通道消融网格从 GAT-GRU 扩到 EGCN（唯一变量仍是 `dataset.use_attr_degree: true`，`degree_scope: train_only`，其余与 `egcn.yaml` 逐字相同）。要点（已写进该 config 文件头）：① 模型侧零改动——共享抽取器的 `input_dim` 由 `dynamic_data.x.size(1)` 自动得出（701→702），`hidden_dims: ["auto",...]` 同理；② X 宽变 ⇒ 所有 d=701 的 ckpt / `backbone.pth` 失效，`load_state_dict(strict=False)` 会在形状不匹配处直接抛错 ⇒ 必须新 `output_subdir` 且 `pretrained.path: null`，`run_sampling.py` 的 Phase 1 donor 也一并重训（不可 resume 旧 `backbone.pth`）；③ `degree_calibration_out` 留 null 时日后部署只能用 `degree_scope: full`（会有尺度漂移），若准备部署该 ckpt 就先给个 npz 路径——注意该文件存在即跳过覆盖，而划分种子恒为 42 ⇒ 多个 training seed 共用同一列、一份参考文件就够。`common_config.yaml` 的白名单注释同步为 `{gatgru_vec,gatgru_2layer,egcn}_deg*.yaml` | `Models/configs/egcn_deg.yaml`（新）、`Training/common_config.yaml`（注释） | 新增文件；已有行为不变 | §3.5, §7, §9 |
| 2026-09-17 | **漏改签名导致入口 TypeError 的修复**：`dataset_feature_kwargs()` 多了 6 个 degree 键，但两个包装函数没同步签名 ⇒ 远端 `SampleSetting/run_sampling.py` 在 Phase 1 直接崩 `create_dataloaders() got an unexpected keyword argument 'degree_scope'`（`Training/train_*.py`、`Bootstraps/bootstrap_v2_{train,analysis}.py` 同一条路径）。现已给 `create_dataloaders` 与 `create_bootstrap_datasets` 补齐 `degree_scope` / `degree_triples` / `degree_triples_path` / `degree_thinning` / `degree_thinning_seed` / `degree_calibration_path` / `degree_calibration_out` 并透传 feature_kwargs；**根因防范**：`_tmp_dump/degree_scope_selftest.py` 新增第 8 节，用 `inspect.signature` 断言 `CompanySupplyDataset.__init__` / `create_dataloaders` / `create_bootstrap_datasets` 三者都接受 `dataset_feature_kwargs({})` 的全部键（28 项全过）⇒ 以后再加 X 相关的 yaml 键，漏改会在本地自检暴露，而不是在训练跑起来之后 | `Data/company_dataset.py`、`_tmp_dump/degree_scope_selftest.py`、`TechnicalGuide.md` | `create_dataloaders` / `create_bootstrap_datasets` 各新增 7 个带默认值的形参（默认值与 common 一致 ⇒ 老调用方零改动）；行为不变（默认 `use_attr_degree: false` 时不触发任何度逻辑） | §3.1, §3.5, §9 |
| 2026-09-17 | 文档同步：§3.5 整体重写（定义 / scope 表 / 部署期为何用全图 / train→deploy 漂移与两种处理 / 陷阱与遗留 limitation）、§7 度那一行改为 `use_attr_degree + degree_scope` 且标注 imputation 必须覆盖、`TechnicalGuide.md` §9 新增本行 | `TechnicalGuide.md` | 纯文档 | §3.5, §7, §9 |
