...
MODEL_METRIC_TAGS = {
    'bigru': {
        ...
        'best_criterion': 'Test/Epoch_AUC',  # preferred metric for best-epoch selection
        'best_direction': 'max',
    },
    'egcn': {
        'train_loss': 'Train/Epoch_BCE',
...
def plot_summary_table(best_df, epoch_df):
    ...
    key_tags = [
        ...
        'Test/Epoch_Loss',
        'Train/Epoch_BCE',
        'Monitor/Factset_Quantile',
        'Monitor/Wasserstein_Diff',
    ]
...
def plot_config_legend():
    """Draw the parameter-configuration legend figure"""
    fig, ax = plt.subplots(figsize=(14, 3.5))
    ax.axis('off')

    configs = [
        ('ftt', 'filter_factset_neg=F, intra_industry_neg=T, use_pred_neg=T',
         'no avoidance of neg samples on FactSet edges, intra-industry neg sampling on (hardest)'),
        ('ftf', 'filter_factset_neg=F, intra_industry_neg=T, use_pred_neg=T',
         'no avoidance of neg samples on FactSet edges, intra-industry neg sampling on'),
        ('fff', 'filter_factset_neg=F, intra_industry_neg=F, use_pred_neg=T',
         'no avoidance of neg samples on FactSet edges, intra-industry neg sampling off'),
        ('tff', 'filter_factset_neg=T, intra_industry_neg=F, use_pred_neg=T',
         'avoidance of neg samples on FactSet edges, intra-industry neg sampling off (easiest)'),
    ]

    colors = get_model_colors()
    for i, (flag, params, desc) in enumerate(configs):
        y_pos = 0.85 - i * 0.20
