"""
GAT-GRU unidirectional-temporal ablation version (UniGRUImputationWithGATVectorized)

Purpose: bidirectional vs. unidirectional GRU ablation, isolating the temporal-direction variable.
The only differences from BiGRUImputationWithGATVectorized in gatgru_vectorized.py:
    temporal_encoder uses nn.GRU(bidirectional=False), keeping only the forward direction;
    hence dynamic_embeddings has dimension [T, N, dynamic_hidden_dim],
    and edge_predictor has input dimension 2*dynamic_hidden_dim+1.
...
class UniGRUImputationWithGATVectorized(nn.Module):
    """GAT-GRU unidirectional (Vectorized): precomputed edge indices + GATEncoderVectorized + forward-only GRU"""
    ...
    def __init__(self, dynamic_data: Data, static_hidden_dim, dynamic_hidden_dim,
        ...
        # Dynamic temporal encoder (unidirectional GRU, the ablation variable)
        ...
        # Edge predictor (input dimension is halved by the unidirectional encoding)
        self.edge_predictor = nn.Sequential(
            nn.Linear(2 * dynamic_hidden_dim + 1, 256),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(256, 128),
            nn.ReLU(),
    ...
    def forward(self, node_pairs, time_indices):
        ...
        static_sequence = self.static_encoder(self.edge_index_list, self.raw_node_feats)

        # Unidirectional GRU temporal encoding
        gru_input = static_sequence.transpose(0, 1)  # [N, T, static_hidden_dim]
        dynamic_sequence, _ = self.temporal_encoder(gru_input)
        dynamic_embeddings = dynamic_sequence.transpose(0, 1)  # [T, N, dynamic_hidden_dim]
