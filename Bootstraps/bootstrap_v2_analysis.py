...
def extract_test_data_with_graph_indices(bootstrap_dir, batch_size=128):
    ...
    from Data.company_dataset import create_bootstrap_datasets, dataset_feature_kwargs
...
def main():
    ...
    if not os.path.exists(model_config_path) and not os.path.exists(model_config_name):
        # Backward compatibility: may be a full path directly; otherwise use the default config
        print(f"  [WARN] Model config not found: {model_config_name}, using gatgru_vec")
        model_config_name = 'gatgru_vec'
    # Merge Training/common_config.yaml like Training/config_loader.load_config, so the parts shared
    # by every backbone (e.g. the node-feature extractor) resolve exactly as during training.
    cfg = load_config(model_config_name)
