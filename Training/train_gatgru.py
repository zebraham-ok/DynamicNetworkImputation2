"""
GAT-GRU single-run training script
Usage: python Training/train_gatgru.py [--config gatgru_vec] [--device cuda:0]
      If --config is omitted, defaults to Models/configs/gatgru_vec.yaml (vectorized version)
"""
import sys
import os
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='GAT-GRU single-run training')
    parser.add_argument('--config', '-c', default='gatgru_vec',
                        help='model config name (default: gatgru_vec, vectorized version)')
